...
(function () {
  ...
  function renderGlobalProgress() {
    let done = 0, total = 0;
    PAGES.forEach(p => { done += pageCheckDone(p); total += pageCheckItems(p); });
    ...
    const t = $('#globalProgressTxt');
    if (t) t.textContent = pct + '%';
    const chip = $('#globalProgress');
    if (chip) chip.title = '전체 진행률 ' + done + ' / ' + total + '개 완료';
  }
  ...
  const PF_DEFAULT = { school: '○○중학교', dept: '과학정보부' };
  ...
  function wRun(text, o) {
    if (!text) return '';
    return '<w:r>' + wRunProps(o) + '<w:t xml:space="preserve">' + xesc(text) + '</w:t></w:r>';
  }
  ...
  function wInline(node, base) {
    ...
    node.childNodes.forEach(n => {
      if (n.nodeType === 3) {
        ...
        if (tag === 'small') { out += '<w:r><w:br/></w:r>' + wInline(n, base); return; }
        // 하단 배너 둘째 줄
        if (tag === 'i' && (' ' + (n.parentNode.className || '') + ' ').indexOf(' ft-tx ') >= 0) {
          out += '<w:r><w:br/></w:r>' + wInline(n, base); return;
        }
        // 표 머리글 안의 ※ 단서는 줄을 내려서 작게
        if (tag === 'em' && n.parentNode.tagName && n.parentNode.tagName.toLowerCase() === 'th') {
          out += '<w:r><w:br/></w:r>' +
                 wInline(n, Object.assign({}, base, { b: false, sz: 16, color: '555555' }));
          return;
        }

        // 빈 밑줄 칸(.ln) — 워드에서는 밑줄 친 공백으로
        if (cl.indexOf(' ln ') >= 0) {
          const wide = cl.indexOf(' w3 ') >= 0;
          out += wRun(wide ? '                    ' : '          ',
                      Object.assign({}, base, { u: true }));
          return;
        }
        out += wInline(n, o);
        // 체크 항목(.cb)은 서로 붙지 않게 간격
        if (cl.indexOf(' cb ') >= 0) out += wRun('      ', base);
        // 꼬리말·표지의 span들 사이 간격
  ...
  function wTable(tblEl) {
    ...
    rows.forEach(tr => {
      ...
      Array.prototype.slice.call(tr.children).forEach(td => {
        ...
        x += '<w:tc><w:tcPr><w:tcW w:w="0" w:type="auto"/>' +
             (head ? '<w:shd w:val="clear" w:color="auto" w:fill="EDF3F9"/>' : '') +
             '<w:vAlign w:val="center"/></w:tcPr>' +
             wPara(wInline(td, { b: head, sz: 18 }), { before: 20, after: 20 }) +
             '</w:tc>';
      });
      x += '</w:tr>';
    });
    return x + '</w:tbl>' + wPara('', { after: 80 });
  }

  /* 생성된 인쇄용 HTML → WordprocessingML */
  function htmlToWml(html) {
    ...
    roots.forEach((page, pi) => {
      ...
      if (hd) {
        ...
        if (sm) out += wPara(wInline(sm, { sz: 22, color: '5A6674' }), { align: 'center' });
        if (tt) out += wPara(wInline(tt, { b: true, sz: 56, color: '0F6FB8' }), { align: 'center', after: 200 });
      }
      ...
      if (ct) {
        out += wPara(wInline(cs || doc.createElement('i'), { b: true, sz: 24, color: '0F6FB8' }), { align: 'center', before: 1200 });
        out += wPara(wInline(ct, { b: true, sz: 64 }), { align: 'center', after: 240 });
        if (cd) out += wPara(wInline(cd, { sz: 22, color: '5A6674' }), { align: 'center', after: 600 });
        if (cf) out += wPara(wInline(cf, { b: true, sz: 30 }), { align: 'center' });
      }
      ...
      if (ph) {
        ...
        out += wPara(wInline(b || ph, { b: true, sz: 32, color: '0F6FB8' }), { after: 160 });
      }

      // 반납 확인서 제목
      const h1 = page.querySelector('h1');
      if (h1) out += wPara(wInline(h1, { b: true, sz: 36, u: true }), { align: 'center', after: 100 });
      const sub = page.querySelector('.sub');
      if (sub) out += wPara(wInline(sub, { sz: 22 }), { align: 'center', after: 260 });
      page.querySelectorAll('.who > div').forEach(w => {
        out += wPara(wInline(w, { sz: 22 }), { align: 'right', after: 60 });
      });
      ...
      const walk = el => {
        Array.prototype.slice.call(el.children).forEach(n => {
          ...
          const c = ' ' + cl + ' ';
          if (tag === 'table') { out += wTable(n); return; }

          // 공문 서식(계획안·심의안) 요소
          if (c.indexOf(' doc-h ') >= 0) {
            out += wPara(wInline(n, { b: true, sz: 40 }), { align: 'center', before: 200, after: 80 });
            return;
          }
          if (c.indexOf(' doc-sub ') >= 0) {
            out += wPara(wInline(n, { b: true, sz: 26 }), { align: 'center', after: 260 });
            return;
          }
          if (c.indexOf(' meta ') >= 0) {
            out += wPara(wInline(n, { sz: 21 }), { align: 'right', after: 240 });
            return;
          }
          if (c.indexOf(' h1n ') >= 0) {
            const em = n.querySelector('em'), b = n.querySelector('b');
            out += wPara(wRun((em ? em.textContent + '. ' : ''), { b: true, sz: 28, color: '0F6FB8' }) +
                         (b ? wInline(b, { b: true, sz: 28 }) : ''),
                         { before: 240, after: 100 });
            return;
          }
          if (c.indexOf(' h2n ') >= 0) {
            out += wPara(wInline(n, { b: true, sz: 23 }), { before: 180, after: 60, ind: 100 });
            return;
          }
          if (c.indexOf(' dot ') >= 0) {
            out += wPara(wRun('◦ ', { sz: 20, color: '0F6FB8' }) + wInline(n, { sz: 20 }),
                         { ind: 340, hang: 180, after: 40 });
            return;
          }
          if (c.indexOf(' dash ') >= 0) {
            out += wPara(wRun('- ', { sz: 19, color: '7A8894' }) + wInline(n, { sz: 19, color: '333333' }),
                         { ind: 560, hang: 180, after: 30 });
            return;
          }
          if (cl.indexOf('sec') >= 0 && tag === 'div') {
            const h2 = n.querySelector('h2');
            if (h2) out += wPara(wInline(h2, { b: true, sz: 26, color: 'FFFFFF' }), { shd: '0F6FB8', after: 60 });
          ...
          if (cl.indexOf('kv') >= 0) {
            ...
            out += wPara(wRun((b ? b.textContent : '') + ' : ', { b: true, sz: 20, color: '0F6FB8' }) +
                         (v ? wInline(v, { sz: 20 }) : ''), { ind: 200, after: 30 });
            return;
          }
  ...
  const R = {
      ...
      (b.items || []).forEach(l => {
        if (l.url) {
          h += '<a class="lk" href="' + l.url + '" target="_blank" rel="noopener noreferrer">' +
               ...
               '<span class="lk-body"><b>' + l.name + '</b><small>' + (l.desc ? l.desc + ' · ' : '') + l.url + '</small></span>' +
    ...
    profile(b) {
      ...
      (b.groups || []).forEach(g => {
        ...
        const rest = (g.fields || []).filter(f => f.type !== 'seg');
    ...
    swgen(b) {
      ...
      h += '<label class="rg-f rg-rules"><span>에듀테크 소프트웨어 목록 ' +
           '<em>— 한 줄에 하나씩 <b>제품명 | 관련 교과 | 활용 목적</b> (뒤는 생략 가능)</em></span>' +
           '<textarea data-s="swList" rows="12" spellcheck="false"></textarea></label>';

      h += '<div class="rg-actions"><button type="button" class="rg-btn ghost" id="swReset">처음 예시로 되돌리기</button></div>';
    ...
    cabgen(b) {
      ...
      h += '<div class="rg-pick"><div class="rg-row"><span class="rg-lb">만드는 방법</span>' +
           ...
           '<button type="button" data-v="gpt"><b>ChatGPT 이미지</b><small>그림 안내문 프롬프트</small></button>' +
           '<button type="button" data-v="flow"><b>Google Flow 이미지</b><small>그림 안내문 프롬프트</small></button>' +
           '</div></div></div>';
    ...
    wifigen(b) {
      ...
      h += '<div class="wf-only many">' +
           '<div class="wf-xl">' +
             ...
             '</div>' +
             '<div class="wf-xl-msg" id="wfMsg">첫 줄은 머리글로 보고 건너뜁니다. ' +
             '<b>A열 구역 · B열 네트워크 이름 · C열 비밀번호</b> 순서로 적으세요.</div>' +
           '</div>' +
  ...
  function renderHome() {
    ...
    h += '<div class="sec-head"><h2>자료 넣는 방법</h2><span>GitHub에 그대로 올려도 동작합니다</span></div>' +
         ...
         '(첨부가 많은 업무는 <code>학운위/</code> <code>매뉴얼/</code> <code>계정/</code> <code>mdm/</code> 하위 폴더로 정리되어 있습니다)</li>' +
...
'NEGATIVE PROMPT: misspelled Korean, garbled or invented glyphs, extra random text, watermark, ' +
  ...
  function slipPage(st, label) {
    ...
    return '<div class="page">' +
      ...
      '<div class="signs">' +
        '<div class="srow"><span>학생 :</span><span class="ln w3"></span><em>(인 또는 서명)</em>' +
          '<span class="gap"></span><span>학부모 :</span><span class="ln w3"></span><em>(인 또는 서명)</em></div>' +
        '<div class="srow right"><span>담당 :</span><span class="ln w3"></span><em>(인 또는 서명)</em></div>' +
      '</div>' +
...
h1{font-family:'Gothic A1',sans-serif;font-size:7.6mm;font-weight:800;text-align:center;
 letter-spacing:-.02em;text-decoration:underline;text-underline-offset:2mm;margin-bottom:4mm}
.sub{text-align:center;font-size:4mm;margin-bottom:11mm}
.who{display:flex;flex-direction:column;align-items:flex-end;gap:3mm;font-size:4.2mm;margin-bottom:7mm}
.ln{display:inline-block;border-bottom:.35mm solid #000;height:4.6mm;vertical-align:-1mm}
.w1{width:12mm}.w3{width:30mm}
table{width:100%;border-collapse:collapse;border:.5mm solid #000}
th,td{border:.35mm solid #000;padding:2.6mm 3mm;font-size:3.9mm;text-align:left;vertical-align:middle}
th{width:60mm;font-weight:600;line-height:1.45;word-break:keep-all}
th em{display:block;font-style:normal;font-size:3.1mm;color:#333;margin-top:.8mm;white-space:nowrap}
.parts{display:grid;grid-template-columns:1fr 1fr;gap:1.8mm 3mm}
.yn{display:grid;grid-template-columns:1fr 1fr;gap:3mm}
.cb{font-size:3.9mm;white-space:nowrap}
.notice{font-size:3.6mm;line-height:1.55;margin:6mm 0 11mm;padding-left:5mm;text-indent:-5mm}
.body{font-size:4.2mm;line-height:1.7;margin-bottom:14mm;text-indent:4mm}
.signs{display:flex;flex-direction:column;gap:5mm;font-size:4.2mm;margin-bottom:16mm}
.srow{display:flex;align-items:flex-end;gap:2mm;flex-wrap:wrap}
.srow.right{justify-content:flex-end;padding-right:2mm}
.srow em{font-style:normal;font-size:3.4mm}
.srow .gap{flex:1;min-width:6mm}
.date{text-align:center;font-size:4.6mm;letter-spacing:.02em;margin-bottom:5mm}
.school{text-align:center;font-family:'Gothic A1',sans-serif;font-size:6.4mm;font-weight:800;letter-spacing:.06em}
.dept{text-align:center;font-size:3.8mm;color:#333;margin-top:1.5mm}
`;
    let body;
    if (st.copies === '2') {
  ...
  function wfLoad() {
    ...
    st.school = pf.school || st.school;
    st.dept = pf.dept || st.dept;
    st.contact = st.contact || pfContact(pf);
  ...
  function wifiPage(st) {
    ...
    return '<div class="page">' +
      ...
      '<div class="ft"><span class="ft-tx">문의 : ' +
        esc([st.dept, st.contact].filter(Boolean).join(' · ') || '학교 담당 부서') +
      '</span></div>' +
    '</div>';
  }
...
@media print{.page{page-break-after:auto}}`;
  ...
  function wfInit() {
    ...
    wrap.addEventListener('click', e => {
      ...
      if (e.target.closest('#wfTpl')) {
        downloadXlsx('와이파이_구역별_정리양식.xlsx', [{
          name: '와이파이 목록',
          cols: [22, 26, 20, 40],
          rows: [
    ...
    if (fileEl) fileEl.addEventListener('change', async e => {
      ...
      try {
        ...
        if (!lines.length) { say('읽을 수 있는 줄이 없습니다. A열 구역 · B열 네트워크 이름 · C열 비밀번호로 정리했는지 확인하세요.', 'bad'); return; }
        st.rooms = lines.join('\n');
        const ta = $('[data-f="rooms"]', wrap);
        if (ta) ta.value = st.rooms;
        pfSet({ wifi: st.rooms });
        wfSave(st); wfRender(st);
        say('<b>' + file.name + '</b> — ' + lines.length + '곳을 넣었습니다. 아래 목록에서 바로 고칠 수 있습니다.', 'ok');
      } catch (err) {
        say('읽지 못했습니다 — ' + esc(err.message || String(err)) + ' 엑셀이 열려 있다면 닫고 다시 시도하거나, CSV로 저장해 올려 보세요.', 'bad');
      }
      e.target.value = '';
    });
  ...
  function csPage(st) {
    ...
    return '<div class="page">' +
      ...
      '<div class="ft"><span class="ft-tx">문의 : ' +
        esc([st.dept, st.contact].filter(Boolean).join(' · ') || '학교 담당 부서') + '</span></div>' +
    '</div>';
  }
...
.signs{display:flex;flex-direction:column;gap:5mm;font-size:4mm;margin-bottom:10mm}
.srow{display:flex;align-items:flex-end;gap:1.5mm;flex-wrap:wrap}
.srow em{font-style:normal;font-size:3.3mm;color:#555}
.ln{display:inline-block;border-bottom:.35mm solid #000;height:4.4mm}
.w1{width:11mm}.w3{width:34mm}
...
.ft-tx{font-size:3.8mm;color:#5A6674}`;
  ...
  function cbPrompt(st) {
    ...
    const dept = [st.dept, st.contact].filter(Boolean).join(' · ') || '학교 담당 부서';
...
`;
  ...
  function eduRender(kind) {
    const out = $('#eduOut');
    if (!out) return;
    const p = pfGet();

    $$('#eduGen .rg-seg').forEach(seg => {
      $$('button', seg).forEach(btn => btn.classList.toggle('on', btn.getAttribute('data-v') === kind));
    });
    ...
    const want = [
      ...
      ['lossRule', '분실 시 부담'], ['wifi', '와이파이 목록'], ['penalty', '벌점 내규']
    ];
    const miss = want.filter(w => !pfv(p, w[0])).map(w => w[1]);
  ...
  function eduInit() {
    ...
    if (!wrap) return;
    let kind = 'book';
    try { kind = localStorage.getItem(EDU_KEY) || 'book'; } catch (e) {}

    wrap.addEventListener('click', e => {
      const seg = e.target.closest('.rg-seg button');
      if (seg) {
        kind = seg.getAttribute('data-v');
        try { localStorage.setItem(EDU_KEY, kind); } catch (e2) {}
        eduRender(kind);
        return;
      }
  ...
  // 소프트웨어 목록 파싱: "이름 | 교과 | 활용 목적" (뒤는 생략 가능)
  function swItems(st) {
    return pfRows(st.swList).map(r => ({
      name: r[0] || '', subject: r[1] || '전 교과', purpose: r[2] || ''
    })).filter(x => x.name);
  }
